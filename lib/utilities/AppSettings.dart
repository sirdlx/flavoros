...
class AppSettingsModel with ChangeNotifier {
  ...
  void pushTo(String path, {DataItem data, BuildContext context}) {
  ...
  void pushToReplace(String path, {DataItem data, BuildContext context}) {
    Size size = MediaQuery.of(context).size;
    if (size.width > 950) {
      setCurrentPage(path);
    } else {
  ...
  AppSettingsModel(BuildContext context, {this.device, this.routes}) {
    print(this.routes);
